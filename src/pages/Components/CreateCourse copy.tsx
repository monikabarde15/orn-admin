...
const mockApi = {
  createCourse: async (data: CourseFormData): Promise<{ id: string; success: boolean }> => {
    ...
    return { id: `course-${Date.now()}`, success: true }
  },
  addModule: async (_courseId: string, _module: Omit<Module, "id">): Promise<{ id: string; success: boolean }> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    return { id: `module-${Date.now()}`, success: true }
  },
  updateModule: async (_moduleId: string, _data: Partial<Module>): Promise<{ success: boolean }> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    return { success: true }
  },
  ...
  addLesson: async (_moduleId: string, _lesson: Omit<Lesson, "id">): Promise<{ id: string; success: boolean }> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    return { id: `lesson-${Date.now()}`, success: true }
  },
  updateLesson: async (_lessonId: string, _data: Partial<Lesson>): Promise<{ success: boolean }> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    return { success: true }
  },
