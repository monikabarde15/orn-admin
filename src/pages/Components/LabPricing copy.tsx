...
useEffect(() => {
  const fetchPackages = async () => {
    try {
      ...
      data.forEach(pkg => {
        const base = pkg.name.split("-")[0].trim();
      ...
      setAllPackages(grouped);   // store indexed data
      setBillingType("free");

      loadLabs("free", grouped); // instant lab load

    } catch (err) {
      console.log("Package load failed", err);
    }
  };
...
const loadLabs = (type, data = allPackages) => {
  if (!data) return;

  const result = Object.keys(data).map(labName => {
    const pkg = data[labName][type];
    if (!pkg) return null;

    return {
      name: labName,
      newdescription: pkg.description,
      monthlyPrice: data[labName]?.monthly?.price,
      yearlyPrice: data[labName]?.yearly?.price,
      planId: pkg.id,
      subscription: pkg.subscription,
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (newlyLaunched) {
            ...
            window.open(`${window.location.origin}/lab?user=${userId}&lab=${lab}`, "_blank");
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight bg-gradient-to-r from-purple-400 to-blue-300 bg-clip-text text-transparent mb-4">
           Real-Time High Availability Cluster Lab Online
          </h1>
